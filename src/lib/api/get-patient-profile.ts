...
export const getPatientProfile = cache(async (patientId: string) => {
	...
	return getPatientProfileForOrganization(patientId, organizationId);
});

export async function getPatientProfileForOrganization(patientId: string, organizationId: string) {
	"use cache";
	cacheLife("max");
	cacheTag(`patient-profile-${organizationId}-${patientId}`);
	...
	const [profile] = await db
		...
		.leftJoin(patientEmergencyContact, eq(patient.id, patientEmergencyContact.patientId))
		.leftJoin(patientPhysicalInformation, eq(patient.id, patientPhysicalInformation.patientId))
