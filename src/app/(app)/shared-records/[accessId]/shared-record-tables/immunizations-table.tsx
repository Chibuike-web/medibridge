...
import { getSharedImmunizationsTableAction } from "../shared-record-table-actions";
...
const immunizationDateFilterPresets = [
  {
    label: "Today",
    getRange: (today: Date) => ({
...
export type SharedImmunizationRow = {
  vaccineName: string;
  dose: string;
...
export function SharedImmunizationsTable({
  accessId,
  rows,
}: {
  accessId: string;
  rows: SharedImmunizationRow[];
}) {
  const [tableRows, setTableRows] = useState(rows);
  const [recordSearchQuery, setRecordSearchQuery] = useState("");
  const [statusFilters, setStatusFilters] = useState<
    SharedImmunizationRow["status"][]
  >([]);
  ...
  function refreshTable({
    ...
    nextQuery?: string;
    nextStatusFilters?: SharedImmunizationRow["status"][];
    nextCreatedFrom?: string;
    nextCreatedTo?: string;
  }) {
    const requestId = ++latestRequestIdRef.current;
    startTransition(async () => {
      const nextRows = await getSharedImmunizationsTableAction({
        accessId,
        query: nextQuery,
    ...
    () => [
      {
        id: "select",
        header: () => (
          <Checkbox aria-label="Select all immunizations" disabled />
        ),
        cell: ({ row }) => (
          <Checkbox
            aria-label={`Select ${row.original.vaccineName}`}
            disabled
          />
        ),
        enableSorting: false,
      ...
      },
      {
        ...
        enableSorting: true,
        cell: ({ row }) => (
          <span className="font-medium">{row.original.vaccineName}</span>
        ),
      },
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-gray-900">Immunizations</h2>
...
function ImmunizationFilterMenu({
  activeFilterSubmenu,
  statusFilters,
  createdFrom,
  createdTo,
  ...
  activeFilterSubmenu: ImmunizationFilterSubmenu | null;
  statusFilters: SharedImmunizationRow["status"][];
  createdFrom: string;
  createdTo: string;
  isPending: boolean;
  onActiveFilterSubmenuChange: (
    submenu: ImmunizationFilterSubmenu | null,
  ) => void;
  onStatusFiltersChange: (values: SharedImmunizationRow["status"][]) => void;
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <CheckboxFilterList
              name="shared-immunization-status"
              options={immunizationStatusFilterOptions}
...
function DatePresetList({
  ...
  return immunizationDateFilterPresets.map((preset) => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        ...
        aria-label="Open immunization actions"
      >
