...
export function SharedImmunizationsTable({ rows }: { rows: SharedImmunizationRow[] }) {
	...
	const filteredRows = useMemo(() => {
		...
		return rows.filter(
			(row) =>
				row.vaccineName.toLowerCase().includes(normalizedRecordSearchQuery) ||
				row.immunizationId.toLowerCase().includes(normalizedRecordSearchQuery),
		);
			...
			<div className="mt-7 mb-4 flex items-center gap-2">
				<div className="relative min-w-0 flex-1">
					...
					<Input
						...
						placeholder="Search by vaccine and immunization id"
						className="pl-10"
					/>
				</div>
				<Button
