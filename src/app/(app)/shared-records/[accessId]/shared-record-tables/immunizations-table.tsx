...
export function SharedImmunizationsTable({ rows }: { rows: SharedImmunizationRow[] }) {
	...
	const columns = useMemo<ColumnDef<SharedImmunizationRow>[]>(() => [
		{
			id: "select",
			header: () => <Checkbox aria-label="Select all immunizations" disabled />,
			cell: ({ row }) => <Checkbox aria-label={`Select ${row.original.vaccineName}`} disabled />,
			enableSorting: false,
			size: 40,
		},
		{
			...
			enableSorting: true,
			cell: ({ row }) => <span className="font-medium">{row.original.vaccineName}</span>,
		},
	...
	return (
		<div>
			<h2 className="text-xl font-semibold text-gray-900">Immunization</h2>
			<div className="mt-7 mb-4 flex items-center gap-2">
				<div className="relative min-w-0 flex-1">
					...
					<Input type="search" value={recordSearchQuery} onChange={(event) => setRecordSearchQuery(event.target.value)} placeholder="Search by vaccine and immunization id" className="pl-10" />
				</div>
						...
						{table.getHeaderGroups().map((headerGroup) => (
							<TableRow key={headerGroup.id} className="bg-gray-50 hover:bg-gray-50">
								{headerGroup.headers.map((header) => (
									<TableHead key={header.id} style={{ width: header.getSize() }} onClick={header.column.getToggleSortingHandler()} onKeyDown={(event) => { if (event.key === "Enter") header.column.getToggleSortingHandler()?.(event); }} className={cn("h-12 whitespace-nowrap bg-gray-50 px-3 py-0 text-gray-600", header.column.getCanSort() && "cursor-pointer select-none")}>
										<div className="flex items-center justify-between gap-3">
											{header.isPlaceholder ? null : flexRender(header.column.columnDef.header, header.getContext())}
											{header.column.getCanSort() ? <div className="-space-y-2"><RiArrowUpSLine className={cn("size-4 text-gray-800", header.column.getIsSorted() === "desc" ? "opacity-30" : "")} aria-hidden="true" /><RiArrowDownSLine className={cn("size-4 text-gray-800", header.column.getIsSorted() === "asc" ? "opacity-30" : "")} aria-hidden="true" /></div> : null}
										</div>
									</TableHead>
								))}
							</TableRow>
