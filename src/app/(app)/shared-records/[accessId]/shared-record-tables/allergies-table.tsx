...
import { getSharedAllergiesTableAction } from "../shared-record-table-actions";

type AllergyFilterSubmenu = "status" | "severity" | "created-at";
type AllergyDateCompleteRange = { from: Date; to: Date };
...
const allergyDateFilterPresets = [
  {
    label: "Today",
    getRange: (today: Date) => ({
...
export function SharedAllergiesTable({
  accessId,
  rows,
}: {
  accessId: string;
  rows: SharedAllergyRow[];
}) {
  const [tableRows, setTableRows] = useState(rows);
  const [recordSearchQuery, setRecordSearchQuery] = useState("");
  const [statusFilters, setStatusFilters] = useState<
    SharedAllergyRow["status"][]
  >([]);
  const [severityFilters, setSeverityFilters] = useState<
    SharedAllergyRow["severity"][]
  >([]);
  ...
  function refreshAllergiesTable({
    ...
    nextQuery?: string;
    nextStatusFilters?: SharedAllergyRow["status"][];
    nextSeverityFilters?: SharedAllergyRow["severity"][];
    ...
    startTransition(async () => {
      const nextRows = await getSharedAllergiesTableAction({
        accessId,
        query: nextQuery,
  ...
  const debouncedSearch = useDebouncedCallback(
    (nextQuery: string) => refreshAllergiesTable({ nextQuery }),
    300,
  );
  ...
  function handleSeverityFiltersChange(
    nextSeverityFilters: SharedAllergyRow["severity"][],
  ) {
  ...
  const columns = useMemo<ColumnDef<SharedAllergyRow>[]>(
    () => [
      {
        id: "select",
        header: () => <Checkbox aria-label="Select all allergies" disabled />,
        cell: ({ row }) => (
          <Checkbox aria-label={`Select ${row.original.allergen}`} disabled />
        ),
        enableSorting: false,
        size: 40,
      },
      {
        ...
        enableSorting: true,
        cell: ({ row }) => (
          <span className="font-medium">{row.original.allergen}</span>
        ),
      },
      {
        header: "Allergy ID",
      ...
      { header: "Reaction", accessorKey: "reaction", enableSorting: true },
      { header: "Created At", accessorKey: "createdAt", enableSorting: true },
      { header: "Severity", accessorKey: "severity", enableSorting: true },
      {
        header: "Status",
        accessorKey: "status",
        enableSorting: true,
        ...
        >
          ...
          >
            ...
            <FilterSubmenu
              ...
              onOpenChange={(open) =>
                setActiveFilterSubmenu(open ? "severity" : null)
              }
              icon={<RiBarChartBoxLine className="text-lg" />}
              label="Severity"
            >
              <CheckboxFilterList
                name="shared-allergy-severity"
                options={allergySeverityFilterOptions}
                selectedValues={severityFilters}
                isPending={isPending}
                onSelectedValuesChange={handleSeverityFiltersChange}
              />
            </FilterSubmenu>
            <FilterSubmenu
            ...
            >
              <DateFilterContent
                ...
                to={createdTo}
                isPending={isPending}
                onDateRangeApply={handleCreatedAtRangeApply}
              />
            </FilterSubmenu>
          </DropdownMenuContent>
        </DropdownMenu>
...
function DatePresetList({
  ...
  const selectedRange = getDateRangeFromParams(from, to);
  const today = new Date();
  return allergyDateFilterPresets.map((preset) => {
    ...
    return (
      ...
      >
        ...
        {isSameDateRange(selectedRange, presetRange) ? (
  ...
  return (
    <div className="flex min-w-0 flex-col">
      <div className="flex items-center gap-3">
        <DateFieldPlaceholder value={draftRange?.from} label="Start date" />
        <RiArrowRightLine
          className="size-5 shrink-0 text-gray-400"
          aria-hidden
        />
        <DateFieldPlaceholder value={draftRange?.to} label="End date" />
      </div>
      <Calendar
        mode="range"
        selected={draftRange}
...
function isSameDateRange(
  ...
  presetRange: AllergyDateCompleteRange,
) {
  return Boolean(
    range?.from &&
      range.to &&
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        ...
        aria-label="Open allergy actions"
      >
