...
export function SharedMedicationsTable({ rows }: { rows: SharedMedicationRow[] }) {
	...
	const filteredRows = useMemo(() => {
		...
		return rows.filter(
			(row) =>
				row.medication.toLowerCase().includes(normalizedRecordSearchQuery) ||
				row.medicationId.toLowerCase().includes(normalizedRecordSearchQuery),
		);
		...
		() => [
			{
				...
				cell: ({ row }) => <Checkbox aria-label={`Select ${row.original.medication}`} disabled />,
				enableSorting: false,
				size: 40,
			},
			{
				...
				enableSorting: true,
				cell: ({ row }) => <span className="font-medium">{row.original.medication}</span>,
			},
			...
			<div className="mt-7 mb-4 flex items-center gap-2">
				<div className="relative min-w-0 flex-1">
					...
					<Input
						...
						placeholder="Search by medication and medication id"
						className="pl-10"
					/>
				</div>
				<Button
