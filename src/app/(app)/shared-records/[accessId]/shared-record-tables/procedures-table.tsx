...
import { getSharedProceduresTableAction } from "../shared-record-table-actions";
...
const procedureDateFilterPresets = [
  {
    label: "Today",
    getRange: (today: Date) => ({
...
export type SharedProcedureRow = {
  procedure: string;
  indication: string;
  facility: string;
  procedureId: string;
...
export function SharedProceduresTable({
  accessId,
  rows,
}: {
  accessId: string;
  rows: SharedProcedureRow[];
}) {
  const [tableRows, setTableRows] = useState(rows);
  const [recordSearchQuery, setRecordSearchQuery] = useState("");
  const [statusFilters, setStatusFilters] = useState<
    SharedProcedureRow["status"][]
  >([]);
  ...
  function refreshTable({
    ...
    nextQuery?: string;
    nextStatusFilters?: SharedProcedureRow["status"][];
    nextCreatedFrom?: string;
    nextCreatedTo?: string;
  }) {
    const requestId = ++latestRequestIdRef.current;
    startTransition(async () => {
      const nextRows = await getSharedProceduresTableAction({
        accessId,
        query: nextQuery,
  ...
  const columns = useMemo<ColumnDef<SharedProcedureRow>[]>(
    () => [
      {
        id: "select",
        header: () => <Checkbox aria-label="Select all procedures" disabled />,
        cell: ({ row }) => (
          <Checkbox aria-label={`Select ${row.original.procedure}`} disabled />
        ),
        enableSorting: false,
        size: 40,
      },
      {
        ...
        enableSorting: true,
        cell: ({ row }) => (
          <span className="font-medium">{row.original.procedure}</span>
        ),
      },
...
function ProcedureFilterMenu({
  ...
  activeFilterSubmenu: ProcedureFilterSubmenu | null;
  statusFilters: SharedProcedureRow["status"][];
  createdFrom: string;
  createdTo: string;
  isPending: boolean;
  onActiveFilterSubmenuChange: (submenu: ProcedureFilterSubmenu | null) => void;
  onStatusFiltersChange: (values: SharedProcedureRow["status"][]) => void;
...
function DatePresetList({
  ...
  return procedureDateFilterPresets.map((preset) => {
