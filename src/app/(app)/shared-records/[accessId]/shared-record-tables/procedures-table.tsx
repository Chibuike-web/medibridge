...
export function SharedProceduresTable({ rows }: { rows: SharedProcedureRow[] }) {
	...
	return (
		<div>
			...
			<div className="mt-7 mb-4 flex items-center gap-2"><div className="relative min-w-0 flex-1"><RiSearchLine className="pointer-events-none absolute top-1/2 left-3 size-5 -translate-y-1/2 text-gray-400" /><Input type="search" value={recordSearchQuery} onChange={(event) => setRecordSearchQuery(event.target.value)} placeholder="Search by procedure and procedure id" className="pl-10" /></div><Button type="button" variant="outline" className="gap-2 border-gray-200 bg-white text-sm text-gray-600 hover:bg-gray-50 data-[state=open]:border-gray-400 data-[state=open]:ring-4 data-[state=open]:ring-gray-200"><RiFilter3Line aria-hidden="true" />Filter</Button><Button type="button" variant="outline" className="gap-2 border-gray-200 bg-white text-sm text-gray-600 hover:bg-gray-50 data-[state=open]:border-gray-400 data-[state=open]:ring-4 data-[state=open]:ring-gray-200"><RiShare2Line aria-hidden="true" />Export</Button></div>
