...
export function SharedProceduresTable({ rows }: { rows: SharedProcedureRow[] }) {
	...
	const filteredRows = useMemo(() => {
		...
		return rows.filter(
			(row) =>
				row.procedure.toLowerCase().includes(normalizedRecordSearchQuery) ||
				row.procedureId.toLowerCase().includes(normalizedRecordSearchQuery),
		);
		...
		() => [
			{
				...
				cell: ({ row }) => <Checkbox aria-label={`Select ${row.original.procedure}`} disabled />,
				enableSorting: false,
				size: 40,
			},
			{
				...
				enableSorting: true,
				cell: ({ row }) => <span className="font-medium">{row.original.procedure}</span>,
			},
			...
			<div className="mt-7 mb-4 flex items-center gap-2">
				<div className="relative min-w-0 flex-1">
					...
					<Input
						...
						placeholder="Search by procedure and procedure id"
						className="pl-10"
					/>
				</div>
				<Button
