...
export type SharedProcedureRow = { procedure: string; indication: string; facility: string; procedureId: string; createdAt: string; status: "Completed" | "Pending" };

export function SharedProceduresTable({ rows }: { rows: SharedProcedureRow[] }) {
	const [recordSearchQuery, setRecordSearchQuery] = useState("");
	const [sorting, setSorting] = useState<SortingState>([{ id: "procedure", desc: false }]);
	const filteredRows = useMemo(() => {
		const normalizedRecordSearchQuery = recordSearchQuery.trim().toLowerCase();
		if (!normalizedRecordSearchQuery) return rows;
		return rows.filter((row) => row.procedure.toLowerCase().includes(normalizedRecordSearchQuery) || row.procedureId.toLowerCase().includes(normalizedRecordSearchQuery));
	}, [recordSearchQuery, rows]);
	const columns = useMemo<ColumnDef<SharedProcedureRow>[]>(() => [
		{ id: "select", header: () => <Checkbox aria-label="Select all procedures" disabled />, cell: ({ row }) => <Checkbox aria-label={`Select ${row.original.procedure}`} disabled />, enableSorting: false, size: 40 },
		{ header: "Procedure", accessorKey: "procedure", enableSorting: true, cell: ({ row }) => <span className="font-medium">{row.original.procedure}</span> },
		{ header: "Indication", accessorKey: "indication", enableSorting: true },
		{ header: "Facility", accessorKey: "facility", enableSorting: true },
		{ header: "Procedure ID", accessorKey: "procedureId", enableSorting: false, cell: ({ row }) => <CopyIdButton id={row.original.procedureId} /> },
	...
	return (
		<div>
			...
			<div className="mt-7 mb-4 flex items-center gap-2"><div className="relative min-w-0 flex-1"><RiSearchLine className="pointer-events-none absolute top-1/2 left-3 size-5 -translate-y-1/2 text-gray-400" /><Input type="search" value={recordSearchQuery} onChange={(event) => setRecordSearchQuery(event.target.value)} placeholder="Search by procedure and procedure id" className="h-10 pl-10" /></div><Button type="button" size="lg" variant="outline" className="gap-2 border-gray-200 bg-white text-gray-600 hover:bg-gray-50 data-[state=open]:border-gray-400 data-[state=open]:ring-4 data-[state=open]:ring-gray-200"><RiFilter3Line aria-hidden="true" />Filter</Button><Button type="button" size="lg" variant="outline" className="gap-2 border-gray-200 bg-white text-gray-600 hover:bg-gray-50 data-[state=open]:border-gray-400 data-[state=open]:ring-4 data-[state=open]:ring-gray-200"><RiShare2Line aria-hidden="true" />Export</Button></div>
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger
				...
				aria-label="Open procedure actions"
			>
