...
export function SharedImagingTable({ rows }: { rows: SharedImagingRow[] }) {
	...
	const filteredRows = useMemo(() => {
		...
		return rows.filter(
			(row) =>
				row.study.toLowerCase().includes(normalizedRecordSearchQuery) ||
				row.imagingId.toLowerCase().includes(normalizedRecordSearchQuery),
		);
	...
	const columns = useMemo<ColumnDef<SharedImagingRow>[]>(
		() => [
			{
				id: "select",
				header: () => <Checkbox aria-label="Select all imaging studies" disabled />,
				cell: ({ row }) => <Checkbox aria-label={`Select ${row.original.study}`} disabled />,
				enableSorting: false,
				size: 40,
			},
			{
				...
				accessorKey: "study",
				enableSorting: true,
				cell: ({ row }) => <span className="font-medium">{row.original.study}</span>,
			},
			...
			<div className="mt-7 mb-4 flex items-center gap-2">
				<div className="relative min-w-0 flex-1">
					...
					<Input
						...
						placeholder="Search by study and imaging id"
						className="pl-10"
					/>
				</div>
				<Button
