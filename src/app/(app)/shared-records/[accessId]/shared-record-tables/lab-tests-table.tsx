...
import { getSharedLabTestsTableAction } from "../shared-record-table-actions";
...
const labTestDateFilterPresets = [
  {
    label: "Today",
    getRange: (today: Date) => ({
...
export function SharedLabTestsTable({
  accessId,
  rows,
}: {
  accessId: string;
  rows: SharedLabTestRow[];
}) {
  const [tableRows, setTableRows] = useState(rows);
  const [recordSearchQuery, setRecordSearchQuery] = useState("");
  const [statusFilters, setStatusFilters] = useState<
    SharedLabTestRow["status"][]
  >([]);
  const [interpretationFilters, setInterpretationFilters] = useState<string[]>(
    [],
  );
  ...
  function refreshTable({
    ...
    nextFlagFilters = interpretationFilters,
    nextCreatedFrom = createdFrom,
    nextCreatedTo = createdTo,
  }: {
    nextQuery?: string;
    nextStatusFilters?: SharedLabTestRow["status"][];
    nextFlagFilters?: string[];
    nextCreatedFrom?: string;
    nextCreatedTo?: string;
  }) {
    const requestId = ++latestRequestIdRef.current;
    startTransition(async () => {
      const nextRows = await getSharedLabTestsTableAction({
        accessId,
        query: nextQuery,
  ...
  function handleFlagFiltersChange(nextFlagFilters: string[]) {
    setInterpretationFilters(nextFlagFilters);
    refreshTable({ nextFlagFilters });
  }
  ...
  const columns = useMemo<ColumnDef<SharedLabTestRow>[]>(
    () => [
      {
        id: "select",
        header: () => <Checkbox aria-label="Select all lab tests" disabled />,
        cell: ({ row }) => (
          <Checkbox aria-label={`Select ${row.original.test}`} disabled />
        ),
        enableSorting: false,
        size: 40,
      },
      {
        ...
        enableSorting: true,
        cell: ({ row }) => (
          <span className="font-medium">{row.original.test}</span>
        ),
      },
      {
        header: "Lab ID",
  ...
  return (
    <div>
        ...
        >
          ...
          >
            ...
            <FilterSubmenu
              ...
              onOpenChange={(open) =>
                setActiveFilterSubmenu(open ? "flag" : null)
              }
              icon={<RiFlagLine className="size-4.5" />}
              label="Flag"
            >
              <CheckboxFilterList
                name="shared-lab-flag"
                options={labTestFlagFilterOptions}
                selectedValues={interpretationFilters}
                isPending={isPending}
                onSelectedValuesChange={handleFlagFiltersChange}
              />
            </FilterSubmenu>
            <FilterSubmenu
      ...
      interpretationFilters.length > 0 ||
      createdFrom ||
      createdTo ? (
        <div className="mb-4 flex flex-wrap gap-2">
          ...
          {interpretationFilters.map((interpretation) => (
            <FilterPill
              key={interpretation}
              label={`Flag: ${interpretation}`}
              onRemove={() =>
                handleFlagFiltersChange(
                  interpretationFilters.filter(
...
function DatePresetList({
  ...
  return labTestDateFilterPresets.map((preset) => {
