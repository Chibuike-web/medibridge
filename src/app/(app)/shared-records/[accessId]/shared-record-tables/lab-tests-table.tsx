...
export function SharedLabTestsTable({ rows }: { rows: SharedLabTestRow[] }) {
	...
	return (
		<div>
			...
			<div className="mt-7 mb-4 flex items-center gap-2"><div className="relative min-w-0 flex-1"><RiSearchLine className="pointer-events-none absolute top-1/2 left-3 size-5 -translate-y-1/2 text-gray-400" /><Input type="search" value={recordSearchQuery} onChange={(event) => setRecordSearchQuery(event.target.value)} placeholder="Search by test and lab id" className="h-10 pl-10" /></div><Button type="button" size="lg" variant="outline" className="gap-2 border-gray-200 bg-white text-gray-600"><RiFilter3Line aria-hidden="true" />Filter</Button><Button type="button" size="lg" variant="outline" className="gap-2 border-gray-200 bg-white text-gray-600"><RiShare2Line aria-hidden="true" />Export</Button></div>
