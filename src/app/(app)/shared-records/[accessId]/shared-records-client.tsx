...
import { DetailItem, DetailsSection } from "@/features/patients/components/patient-details-section/detail-fields";
...
export function SharedRecordsClient({ patientDetailsSections, records, activeSection }: SharedRecordsClientProps) {
	return (
		<main className="mx-auto w-full max-w-7xl px-6 py-9">
			{activeSection === "patient-details" ? <PatientDetails sections={patientDetailsSections} /> : null}
			{activeSection === "diagnoses" ? <SharedDiagnosesTable rows={records.diagnoses} /> : null}
			{activeSection === "allergies" ? <SharedAllergiesTable rows={records.allergies} /> : null}
			{activeSection === "immunization" ? <SharedImmunizationsTable rows={records.immunizations} /> : null}
			{activeSection === "procedures" ? <SharedProceduresTable rows={records.procedures} /> : null}
			{activeSection === "medications" ? <SharedMedicationsTable rows={records.medications} /> : null}
			{activeSection === "lab-tests" ? <SharedLabTestsTable rows={records.labTests} /> : null}
			{activeSection === "imaging" ? <SharedImagingTable rows={records.imaging} /> : null}
		</main>
	);
}
