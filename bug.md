...
Found while writing behaviour tests for the patient-record section tables on 2026-09-11. Rechecked against the working tree on 2026-09-12. Line numbers may drift as the files change.
...
- Test: covered by a `test.todo` in `imaging-table.test.tsx`.
...
### 9. Chart tooltip always reports "Normal"
...
- Action items with no handler (Export, Archive, Mark as completed, Cancel, Remove) are only asserted for visibility in the current tests. They should become `test.todo` entries naming the expected outcome so the gap stays visible.
