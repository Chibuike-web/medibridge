...
vi.mock("@/lib/api/get-patient-immunizations", () => ({
	getPatientImmunizations: getImmunizationsMock,
}));
...
describe("Transfers API", () => {
	...
	describe("GET /api/transfer-details/:transferId", () => {
		test("returns transfer details for an authenticated request", async () => {
			...
			const response = await getTransferDetails(
				new Request("http://localhost/api/transfer-details/transfer-1"),
				{
		...
		test("returns a 404 null transfer payload when the transfer is missing", async () => {
			...
			const response = await getTransferDetails(
				new Request("http://localhost/api/transfer-details/missing"),
				{
	...
	describe("GET /api/transfer-patient-options", () => {
		test("returns paginated patients using default pagination", async () => {
			...
			const response = await getPatientOptions(
				nextRequest("http://localhost/api/transfer-patient-options"),
			);
	...
	describe("GET /api/transfer-clinical-record-options", () => {
		...
		test("returns 400 when the clinical record type is invalid", async () => {
			const response = await getClinicalRecordOptions(
				nextRequest(
					"http://localhost/api/transfer-clinical-record-options?patientId=patient-1&type=invalid",
				),
			);
		...
		test("maps diagnosis records into transfer options", async () => {
			...
			expect(await response.json()).toEqual({
				records: [
					{ id: "diagnosis-1", name: "Asthma", type: "diagnoses", createdAt: "Jan 10, 2026" },
				],
				page: 2,
				totalRecords: 1,
				totalPages: 1,
		...
		test("maps each remaining clinical record type into transfer options", async () => {
			getAllergiesMock.mockResolvedValue({
				allergies: [{ allergyId: "a1", allergen: "Penicillin", createdAtLabel: "Jan" }],
				totalAllergies: 1,
			});
			getImmunizationsMock.mockResolvedValue({
				immunizations: [{ immunizationId: "i1", vaccineName: "Flu", createdAtLabel: "Jan" }],
				totalImmunizations: 1,
			});
			getProceduresMock.mockResolvedValue({
				procedures: [{ procedureId: "p1", procedure: "Appendectomy", createdAtLabel: "Jan" }],
				totalProcedures: 1,
			});
			getMedicationsMock.mockResolvedValue({
				medications: [{ medicationId: "m1", medication: "Aspirin", createdAtLabel: "Jan" }],
				totalMedications: 1,
			});
			getLabTestsMock.mockResolvedValue({
				labTests: [{ labId: "l1", test: "CBC", createdAtLabel: "Jan" }],
				totalLabTests: 1,
			});
			getImagingMock.mockResolvedValue({
				imagingStudies: [{ imagingId: "x1", study: "CT scan", createdAtLabel: "Jan" }],
				totalImagingStudies: 1,
			});
			...
			for (const [type, record] of expected) {
				const response = await getClinicalRecordOptions(
					nextRequest(
						`http://localhost/api/transfer-clinical-record-options?patientId=patient-1&type=${type}`,
					),
				);
		...
		test("caps the clinical-record limit at 50", async () => {
			...
			await getClinicalRecordOptions(
				nextRequest(
					"http://localhost/api/transfer-clinical-record-options?patientId=patient-1&type=allergies&limit=999",
				),
			);
